...
#         or selected_id in ("openai_compatible", "azure", "ollama")
#     )
#     if show_url:
...
#                 st.error(log)
#             elif "PASSED" in log or "Selected tables" in log or "Module Scope" in log or "Query Cache] HIT" in log:
#                 st.success(log)
#             else:
#                 st.info(log)
...
#         upsert_module,
#     )
#     from db.utils import ALLOWED_TABLES, ensure_database
...
#     tab_query, tab_db, tab_tools, tab_create, tab_modules, tab_schema = st.tabs(
#         [
#             "Ask Question",
#             "Database",
#             "Tools",
...
def apply_sidebar_config() -> None:
    ...
    # if selected_id == "peek":
    #     st.sidebar.caption(
    #         "Peek uses HDFC LiteLLM chat/completions "
    #         "(OpenAI-style model + messages + temperature + max_tokens)."
    #     )
    #     from llm import get_token_usage_summary, reset_token_usage

    #     summary = get_token_usage_summary()
    #     session = summary.get("session") or {}
    #     st.sidebar.subheader("Peek token usage")
    #     c1, c2, c3 = st.sidebar.columns(3)
    #     c1.metric("Prompt", session.get("prompt_tokens", 0))
    #     c2.metric("Completion", session.get("completion_tokens", 0))
    #     c3.metric("Total", session.get("total_tokens", 0))
    #     st.sidebar.caption(f"{session.get('calls', 0)} LiteLLM call(s) this session")
    ...
    with st.sidebar.container():
        ...
        st.markdown('<div class="token-title">Session Token Summary</div>', unsafe_allow_html=True)
...
def main() -> None:
    ...
    tab_query, tab_db, tab_tools, tab_create, tab_table, tab_modules, tab_schema = st.tabs(
        [
            "Ask Question",
            "Database",
            "Tools",
